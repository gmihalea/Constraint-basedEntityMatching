...
public class Matcher {
    ...
    public boolean evaluateConstraint(final ArrayList<String> aValue,
        ...
        // e.g. min1, max3
        for(int i = 0; i < constraintValue.size(); ++i) {
            // All the elements in aValue must be different from the
            // elements in bValue
            if(constraintValue.get(i).equals(Constants.DIFF)) {
                // If the two lists are not entirely different, then
                // the constraint is not respected
                if(!this.checkDiff(aValue, bValue)) {
                    System.out.println("[REASON]:" + constraintValue.get(i) + "["
                            + aValue +  " vs. " + bValue + "]");
                    return false;
                }
            }
            if(constraintValue.get(i).startsWith(Constants.MIN)) {
                // Extract the number that follows the string MIN
                final int count = Integer.parseInt(constraintValue.get(i).substring(Constants.MIN.length()));
            ...
            if(constraintValue.get(i).startsWith(Constants.MAX)) {
                final int count = Integer.parseInt(constraintValue.get(i).substring(Constants.MAX.length()));
            ...
            if(constraintValue.get(i).startsWith(Constants.GMT_MAX)) {
                final int count = Integer.parseInt(constraintValue.get(i).substring(Constants.GMT_MAX.length()));
